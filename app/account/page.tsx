      ...
      <div className="shell py-8 sm:py-12">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black text-stone-950 sm:text-5xl">My orders</h1>
            <p className="mt-2 text-sm font-semibold text-stone-600">{user.email}</p>
          </div>
          <form action={logoutCustomer}>
            <button className="btn-secondary rounded-full">Sign out</button>
          </form>
        </div>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <section className="premium-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link href="/preorder" className="btn-primary rounded-full"><ShoppingBasket className="h-4 w-4" /> New preorder</Link>
            </div>
            <div className="mt-4 space-y-3">
              {orders.map((order: any) => (
                <Link key={order.id} href={`/account/orders/${order.order_number}`} className="block rounded-2xl border border-stone-200 bg-white p-4 shadow-crisp transition hover:-translate-y-0.5 hover:border-leaf-100 hover:bg-leaf-50 hover:shadow-lift">
